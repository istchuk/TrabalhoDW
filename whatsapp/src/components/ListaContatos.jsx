// Componente ListaContatos que recebe props: contatos (lista de contatos), 
// editarContato (função para editar), excluirContato (função para excluir), 
// enviarMensagem (função para abrir WhatsApp) e styles (estilos CSS)
export default function ListaContatos({ contatos, editarContato, excluirContato, enviarMensagem, styles }) {
  return (
    ...
    <div className={styles.adicionarcontato}>
      ...
      {/* Percorre todos os contatos recebidos via props */}
      {contatos.map((c, index) => (
        // Cada contato é representado por uma div separada
        <div key={index} className={styles.contato}>
          ...
          <div>
            ...
            {/* Número do contato */}
            <p>{c.numeroContato}</p>
            {/* Tipo do contato (ex: Amigos, Família, Trabalho) */}
            <p>{c.tipoContato}</p>
          </div>

          {/* Container para os botões de ação do contato */}
